...
contract SPRL2V2_RequestWithdraw_Integrations_Test is Integrations_Test {
    ...
    function test_SPRL2V2_RequestWithdraw() external {
    ...
    function testFuzz_SPRL2V2_RequestWithdraw_Several(uint16 requestTime) external {
        ...
        for (uint16 i = 0; i < requestTime; i++) {
            sprl2v2.requestWithdraw(withdrawAmount);
        }
        assertEq(sprl2v2.balanceOf(users.alice.addr), INITIAL_BALANCE - expectedAmount);
        assertEq(sprl2v2.unlockingAmount(), expectedAmount);

        uint256[] memory requestIds = sprl2v2.findUnlockingIDs(users.alice.addr, 0, false, requestTime);
