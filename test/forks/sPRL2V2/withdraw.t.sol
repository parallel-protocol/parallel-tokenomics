...
contract SPRL2V2_Withdraw_Fork_Test is ForkV2_Test {
    ...
    function setUp() public override {
        ...
        (uint256 deadline, uint8 v, bytes32 r, bytes32 s) =
            _signPermitData(users.alice.privateKey, address(sprl2v2), INITIAL_BALANCE, address(prl));

        (uint256[] memory amountsIn,) =
            sprl2v2.depositPRLAndWeth(INITIAL_BALANCE, INITIAL_BALANCE, exactBptAmount, deadline, v, r, s);
        prlAmountDeposited = amountsIn[0];
        wethAmountDeposited = amountsIn[1];
    }

    modifier requestWithdrawal() {
        sprl2v2.requestWithdraw(exactBptAmount);
        _;
    }

    function test_fork_sPRL2V2_WithdrawBPT() external requestWithdrawal {
        skip(sprl2v2.timeLockDuration());
        uint256 aliceBptBalance = sprl2v2.balanceOf(users.alice.addr);
        uint256[] memory requestIds = new uint256[](1);
        requestIds[0] = 0;
        sprl2v2.withdrawBPT(requestIds, DEFAULT_PENALTY_PERCENTAGE);
        assertEq(sprl2v2.balanceOf(users.alice.addr), 0);
        assertEq(bpt.balanceOf(users.alice.addr), aliceBptBalance + exactBptAmount);
    }

    function test_fork_sPRL2V2_WithdrawPRLAndWeth() external requestWithdrawal {
        skip(sprl2v2.timeLockDuration());
        ...
        (uint256 prlAmount, uint256 wethAmount) =
            sprl2v2.withdrawPRLAndWeth(requestIds, minPrlAmount, minWethAmount, DEFAULT_PENALTY_PERCENTAGE);
