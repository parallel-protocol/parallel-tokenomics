...
contract SPRL2V2_Deposits_Fork_Test is ForkV2_Test {
    ...
    function test_fork_sPRL2V2_DepositBPT() external {
        ...
        bpt.approve(address(sprl2v2), exactBptAmount);
        sprl2v2.depositBPT(exactBptAmount);
        assertEq(sprl2v2.balanceOf(users.alice.addr), exactBptAmount);
        assertEq(bpt.balanceOf(users.alice.addr), 0);
    }

    function test_fork_sPRL2V2_DepositPRLAndWeth() external {
